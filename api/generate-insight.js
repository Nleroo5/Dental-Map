...
Avoid scare language. Focus on opportunity and differentiation.`;
    ...
    const user = [
      ...
      `Invisalign mentions: ${competitor_info?.invisalign_mentions ?? 0}`,
    ...
    if (!r.ok) return res.status(500).json({ error: 'OpenAI error', details: await r.text() });

    const data = await r.json();
    res.status(200).json({ insight: data?.choices?.[0]?.message?.content?.trim() ?? 'No insight generated.' });
  } catch (e) {
